...
// Só existe /api/caminhoes, com id opcional no final.
if (($partes[0] ?? '') !== 'api' || ($partes[1] ?? '') !== 'caminhoes') {
...
switch (true) {
    case $metodo === 'GET' && $id === null:
        ControladorCaminhao::listar();
        break;

    case $metodo === 'GET' && $id !== null:
        ControladorCaminhao::buscar((int) $id);
        break;

    case $metodo === 'POST' && $id === null:
        ControladorCaminhao::criar();
        break;

    case $metodo === 'PUT' && $id !== null:
        ControladorCaminhao::atualizar((int) $id);
        break;

    case $metodo === 'DELETE' && $id !== null:
        ControladorCaminhao::excluir((int) $id);
        break;
